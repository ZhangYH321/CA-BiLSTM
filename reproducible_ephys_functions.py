...
def query(behavior=False, n_trials=400, resolved=True, min_regions=2, exclude_critical=True, one=None, str_query=None,
    ...
    ins_df = pd.read_csv(
        './data_releases/biorxiv_2022_05.csv')

    pids = ins_df[ins_df['level'] >= 2].pid.values
    trajectories = one.alyx.rest('trajectories', 'list', provenance='Planned', django=f'probe_insertion__in,{list(pids)}')
...
def compute_metrics(insertions, one=None, ba=None, spike_sorter='pykilosort', new_metrics=True, save=True):
    ...
    for i, ins in enumerate(insertions):
        ...
        print(i)
        try:
            trials = one.load_dataset(eid,  '_ibl_trials.table.pqt', collection='alf')#attribute=training.TRIALS_KEYS
        ...
        try:
            for region in BRAIN_REGIONS: #记录的是每个session 5个不同区域的统计值
...
def filter_recordings(df=None, max_ap_rms=40, max_lfp_power=-140, min_neurons_per_channel=0.1, min_channels_region=5,
    ...
    if df is None:
        ...
        # merge the two dataframes 表的合并  df之前记录的是根据神经元建立的索引   metrics是根据区域  都还没有做筛选
        df['original_index'] = df.index
        df = df.merge(metrics, on=['pid', 'region', 'subject', 'eid', 'probe', 'date', 'lab'])

    # Region Level
    # no. of channels per region  每个session中每个区域的通道数量要大于min_channels_region  region_hit 才为true
    df['region_hit'] = df['n_channels'] > min_channels_region
    # no. of neurons per region   每个session中每个区域的神经元数量要小于min_neuron_region  low_neurons 才为true
    ...
    if behavior:  #这里没看太懂 添加的behavior信息 behavior为true才行，就是说要是小鼠运动的情况下？
        sum_metrics += ~df['behavior']

    df['include'] = sum_metrics == 0 #通过以上筛选 全部满足条件的include才为true
    ...
    inst_count = df_red.groupby(['institute']).pid.nunique()  #每个实验留下了多少个session(一个pid对应一个session)
    institutes = [key for key, val in inst_count.items() if val >= min_rec_lab]

    for lab in institutes:  #实验室留下的session的个数要大于 min_rec_lab    lab_include才为true
    ...
    for key in df_red.groups.keys():
        ...
        if df_k.iloc[0]['neuron_yield'] > min_neuron_region:
            labreg[key[0]][key[2]] += 1 #每个实验室每个区域session的个数

    for lab, regs in labreg.items():
        for reg, count in regs.items():
            if count >= min_lab_region:#每个实验室每个区域session的个数要大于等于min_lab_region=3  permute_include才为true
    ...
    #df.to_csv(r"C:\Users\Zhang\Desktop\data.csv")
    return df
